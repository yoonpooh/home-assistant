...
async function setControlDevicesAvailable(state, mqttClient) {
    ...
    state.discoveredOutlets.forEach(uniqueId => {
        ...
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/availability`, 'available', { retain: true, qos: 1 }),
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/standby_mode/availability`, 'available', { retain: true, qos: 1 }),
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/standby_power/availability`, 'available', { retain: true, qos: 1 }),
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/current_power/availability`, 'available', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredLights.forEach(uniqueId => {
        ...
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/light/${deviceId}/availability`, 'available', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredTemps.forEach(uniqueId => {
        ...
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/temp/${deviceId}/availability`, 'available', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredElevators.forEach(uniqueId => {
        ...
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/elevator/${deviceId}/availability`, 'available', { retain: true, qos: 1 })
        );
    });
    ...
    if (state.parkingState.carNumberDiscovered) {
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/parking/car_number/availability`, 'available', { retain: true, qos: 1 })
        );
    }
...
async function setControlDevicesUnavailable(state, mqttClient) {
    ...
    state.discoveredOutlets.forEach(uniqueId => {
        const deviceId = uniqueId.replace('commax_outlet_', '');
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/availability`, 'unavailable', { retain: true, qos: 1 }),
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/standby_mode/availability`, 'unavailable', { retain: true, qos: 1 }),
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/standby_power/availability`, 'unavailable', { retain: true, qos: 1 }),
            publishAsync(mqttClient, `${topicPrefix}/outlet/${deviceId}/current_power/availability`, 'unavailable', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredLights.forEach(uniqueId => {
        const deviceId = uniqueId.replace('commax_light_', '');
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/light/${deviceId}/availability`, 'unavailable', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredTemps.forEach(uniqueId => {
        const deviceId = uniqueId.replace('commax_temp_', '');
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/temp/${deviceId}/availability`, 'unavailable', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredFans.forEach(uniqueId => {
        const deviceId = uniqueId.replace('commax_fan_', '');
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/fan/${deviceId}/availability`, 'unavailable', { retain: true, qos: 1 })
        );
    });
    ...
    state.discoveredElevators.forEach(uniqueId => {
        const deviceId = uniqueId.replace('commax_elevator_', '');
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/elevator/${deviceId}/availability`, 'unavailable', { retain: true, qos: 1 })
        );
    });
    ...
    if (state.parkingState.carNumberDiscovered) {
        publishPromises.push(
            publishAsync(mqttClient, `${topicPrefix}/parking/car_number/availability`, 'unavailable', { retain: true, qos: 1 })
        );
    }
